...
export const removeFromCart = (itemId) => async (dispatch, getState) => {
  try {
    ...
    const customerDetails = customerResponse.data.find(
    ...
    const cartItemsResponse = await axios.get(
      `/store/carts/${customerDetails.cart_id}/items/`,
      config
    );
    console.log("Cart Items:", cartItemsResponse.data);

    const cartItem = cartItemsResponse.data.find(
    ...
    await axios.delete(
      `/store/carts/${customerDetails.cart_id}/items/${cartItem.id}/`,
      config
    );
